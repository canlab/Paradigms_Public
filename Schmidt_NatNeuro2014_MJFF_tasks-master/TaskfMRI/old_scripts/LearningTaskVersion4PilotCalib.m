...
resultname=strcat('ILtestVersion4PilotCalibSub',num2str(nsub),'Session',num2str(nsession));
...
resultname2=strcat('jitterSession',num2str(nsession),'.mat');
load (resultname2)
...
% data to save
data=zeros(totaltrial,9);
vectordata=zeros(16,4);
timedata=zeros(totaltrial,8);
...
%wait_trigger and first 3 TRs from scanner, launches at 4th 't'
%corresponding to the 4th TR
...
data=[session' trial' pair' checktime1' checktime2' resptime(2:33)' choice' response' feedback'];
vectordata=[gainup; gain; lossup; loss].';
timedata=[time1' startfix' jitter bonus(2:33)' checktime1' startchoic' checktime2' time2'];
