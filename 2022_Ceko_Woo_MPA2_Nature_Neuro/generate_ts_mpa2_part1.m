function [ts, exp, post_q] = generate_ts_mpa2_part1
...
% S1{2} = repmat({'0010'}, 16, 1);
S1{2} = repmat({'overall_avoidance'}, 20, 1);
S1{3} = repmat({'0'}, 20, 1);
S1{4} = repmat({'3', '11'; '5', '9'; '7', '7'}, 7, 1);
...
post_intunp_battery = {{'overall_thermal_int', 'overall_thermal_unp'}, {'overall_pressure_int', ...
            'overall_pressure_unp'}, {'overall_negvis_int', 'overall_negvis_unp'}, ...
            {'overall_negaud_int', 'overall_negaud_unp'}, {'overall_posvis_int', ...
            'overall_posvis_ple'}};
...
for i = run_n
    ...
    if i == 1 || i == 3 || i == 6
        post_q{i} = {'overall_relaxed', 'overall_attention', 'overall_boredness', ...
            'overall_alertness', 'overall_mood', 'overall_comfortness'};
        for j = 1:numel(rand_post_intunp_battery)
