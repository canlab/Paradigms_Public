...
while 1
    Screen('TextSize',window,72);
    DrawFormattedText(window,'Practice','center','center',FGCOLOR);
    timing.initialized = Screen('Flip',window);
    
    [presstime keycode delta] = KbWait;
    if keycode(key.y)
        ntrain = 2;
    elseif keycode(key.n)
        break;
    end
    
    for i = 1:ntrain
        % cue
        Screen('CopyWindow',dspl.cuescreen(randi(3)).w,window);
        tmp.cue = Screen('Flip',window);
        WaitSecs(DURATION.cue);
        
        % DELAY 1 + STIM + DELAY 2
        cursor.x = cursor.xmin;
        while GetSecs < tmp.cue + DURATION.cue + DURATION.trialwithoutratings
        ...
        % OVERALL RATINGS
        tmp.endfix = randi(3)+1;
        tmp.endfix = [tmp.endfix 5-tmp.endfix];
        for ratingtype = Shuffle([1 2])
            % rating cue screen
            Screen('TextSize',window,40);
            DrawFormattedText(window,...
                sprintf('OVERALL %s RATING',RATINGTITLES{ratingtype}),...
                'center',dspl.ycenter-270,FGCOLOR);
            tmp.ratingcue = Screen('Flip',window);
            % initialize rating
            cursor.x = cursor.xmin; %cursor.center - cursor.start(trial,ratingtype);
            tmp.ratingstart = WaitSecs('UntilTime',tmp.ratingcue + DURATION.ratingcue);
            SetMouse(dspl.xcenter,dspl.ycenter);
            while 1
                % measure mouse movement
                [x y click] = GetMouse;
                ...
                if any(click(mousebuttons))
                    ...
                    % freeze screen
                    WaitSecs(tmp.ratingstart + DURATION.rating - tmp.rating);
                    break
                end
                
                % if run out of time
                if GetSecs >= tmp.ratingstart + DURATION.rating
                    % draw scale
                    Screen('CopyWindow',dspl.oscale(ratingtype).w,window);
                    Screen('Flip',window);
            ...
            Screen('Flip',window);
            WaitSecs(tmp.endfix(ratingtype));
        end
    end
end
clear tmp
...
Screen('TextSize',window,40);
DrawFormattedText(window,'Waiting for trigger','center','center',FGCOLOR);
...
cleanupobj = onCleanup(@() save('crashsave')); %Is supposed to save environment if you have to ctrl c in middle of task
