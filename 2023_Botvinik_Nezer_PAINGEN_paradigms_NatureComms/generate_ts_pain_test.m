function [ts, exp, post_q] = generate_ts_pain_test
...
n_stim = 8;
idx1 = ones(1,n_stim) + [3 3 3 4 4 5 5 5]; %enforce equal numbers of elem by permuting idx
%AU is a standin for a dummy trial (no stim) used for expectancy rating
modality = {'TP','PP'};
dur = {'0012','0010'};
idx2 = [1 1 1 1 2 2 2 2];
idx2 = idx2(randperm(n_stim));
S1{1} = [reshape([repmat({'WA'},1,n_stim); modality(idx2)], n_stim*2, 1) ...
    levels( ...
        reshape( ...
            [ones(1,n_stim); ...
            idx1(randperm(n_stim))], ...
            n_stim*2,1) ...
        )' ...
    reshape([repmat({'0001'},1,n_stim); dur(idx2)],n_stim*2,1)];
