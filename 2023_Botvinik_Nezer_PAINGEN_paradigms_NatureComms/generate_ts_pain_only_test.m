function [ts, exp, post_q] = generate_ts_pain

% [ts, exp] = generate_ts_mpa2_part1

exp.instructions = {'overall_avoidance', 'cont_avoidance'};
rng('shuffle');

%% RUN 1-7
% trial_sequence{run_number}{trial_number} = ...
%          { type, intensity(four digits:string), duration(four digits:string),
%           repetition_number, rating_scale_type, cue_duration,
%           post-stim jitter, inter_stim_interval (from the rating to the 
%           next trial), cue_message, during_stim_message};
levels = {'LV0', 'LV1', 'LV2', 'LV3', 'LV4', 'LV5', 'LV6', 'LV7'};
idx1 = ones(1,8) + [3 3 3 4 4 5 5 5]; %enforce equal numbers of elem by permuting idx
%AU is a standin for a dummy trial (no stim) used for expectancy rating
modality = {'TP','TP'};
idx2 = [1 1 1 1 2 2 2 2];
S1{1} = [reshape([modality(ones(8,1)); modality(idx2(randperm(8)))], 16, 1) ...
    levels( ...
        reshape( ...
            [ones(1,8); ...
            idx1(randperm(8))], ...
            16,1) ...
        )' ...
    repmat({'0000'; '0015'},8,1)];
S1{2} = repmat({'overall_expect'; 'overall_int'}, 8, 1);

% randomize jitter order
dur1 = [8, 5, ...
        8, 3]'; %[pre-expect fixation, post-expect fixation]
% dur1 = [0, 5, 13, 3]';
dur2 = [0 7; 3 7; ...
        0 7; 5 7]; %[post-dummy stim fixation, expect rating; post-pain fixation, pain rating]
idx1 = {[1 2 3 4], [1 4 3 2]};
idx1 = cell2mat([idx1(randperm(2)), idx1(randperm(2))]);
dur1 = mat2cell(int2str(dur1(idx1)),ones(16,1),1);
dur2 = dur2(idx1,:);
[x1 y1] = size(dur2);
dur2 = reshape(mat2cell(int2str(reshape(dur2,x1*y1,1)),ones(x1*y1,1),1),x1,y1);

S1{3} = dur1; %pre stimulus fixation
S1{4} = dur2;

trial_n = 16;
run_n = 1;

for k = 1:numel(S1)
    for i = run_n
        temp = S1{k}(1:trial_n,:);
        switch k
            case 1
                for j = 1:trial_n
                    ts{i}{j}(1) = temp(j,1);
                    ts{i}{j}(2) = temp(j,2);
                    ts{i}{j}(3) = temp(j,3);
                end
            case 2
                for j = 1:trial_n
                    ts{i}{j}(4) = {temp(j)};
                end
            case 3
                for j = 1:trial_n
                    ts{i}{j}(5) = temp(j);
                end
            case 4
                for j = 1:trial_n
                    ts{i}{j}(6) = temp(j,1);
                    ts{i}{j}(7) = temp(j,2);
                end
        end
    end
end


post_intunp_battery = {};

for i = run_n
    
    rand_post_intunp_battery = post_intunp_battery(randperm(numel(post_intunp_battery)));
    for j = 1:numel(rand_post_intunp_battery)
        rand_post_intunp_battery{j} = rand_post_intunp_battery{j}(randperm(numel(post_intunp_battery{j})));
    end
    
    if i == 1 || i == 3 || i == 6
        post_q{i} = {};
        for j = 1:numel(rand_post_intunp_battery)
            post_q{i} = [post_q{i} rand_post_intunp_battery{j}];
        end

    else
        post_q{i} = {};
    end
end

end