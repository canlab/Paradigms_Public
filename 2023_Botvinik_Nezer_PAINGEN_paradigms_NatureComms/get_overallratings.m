...
if strcmp(overall_types,'overall_expect')
    barcolor = [255,255,0];
    textcolor = 1;
else
    barcolor = white;
    ...
    if iscell(rating_types.prompts{i}) % when prompt is too long
        for j = 1:numel(rating_types.prompts{i})
            Screen('DrawText', theWindow, rating_types.prompts{i}{j}, ...
                W/2-promptW{i}{j}/2,H/2-promptH/2-150-(40*(numel(rating_types.prompts{i})+1-j)),textcolor);
        end
    else
        Screen('DrawText', theWindow, rating_types.prompts{i}, W/2-promptW{i}/2,H/2-promptH/2-150,textcolor);
    end
