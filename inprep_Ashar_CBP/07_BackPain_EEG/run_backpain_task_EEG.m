function data = run_backpain_task_EEG(subject,day,pressures)

if nargin<1, subject=9999; end
if nargin<2, day=7; end

if nargin < 3 || length(pressures) ~= 4
    pressures = [0.09 0.12 0.16 0.20];
end
...
try
    ...
    % initialize to more than enough NaNs, to pad out timestamps and data for trials
    participant.data.vas_timestamp = nan(participant.experiment.ntrials, 3000);
    
    % give the subj a few sec to get oriented on the ratings
    InitContinuousVASRatings(5);
    ...
    for t=1:participant.experiment.ntrials
        ...
        RunTrial(t)
        % save data thus far
        save(fullfile(participant.settings.path.tmp,sprintf('sub-%04d_ses-%02d_task-chronic_events_tmp_EEG.mat',subjectnum,day)),'participant');
    end
    ...
    % finish up
    save(fullfile(participant.settings.path.data,sprintf('sub-%04d_ses-%02d_task-chronic_events_EEG.mat',subjectnum,day)),'participant');
    cleanup;  %  will also stop the bladder and close the log file
    
    % plot bladder response, for elig purposes
    plot_continuous_ratings( fullfile(participant.settings.path.data,sprintf('sub-%04d_ses-%02d_task-chronic_events_EEG.mat',subjectnum,day)) );
...
% subfunctions
    function ptbInit
        ...
        if nargin < 3
            participant.experiment.stimset       = [0.09 0.12 0.16 0.20]; % adapt, pressure in kg
        else
        ...
        % Common strings
        participant.settings.ptb.qstr = {'How painful? Please rate continuously'};
        ...
        participant.data.vasrate   = NaN(participant.experiment.ntrials, 25000);
    ...
    function CyclePressure
        ...
        for i=1:3
            fwrite(participant.settings.conf.udp_t, '0.09, t');
            WaitSecs(15.0);
    ...
    function RunTrial(x)
        ...
        participant.data.eventTimes(x).stimOnset = participant.data.eventTimes(x).trialStartTime; %Screen('Flip',participant.settings.window); % fixation on
        
        %pressure increase
        sendUDP(x);
                    
        % get pain rating while balloon is inflated
        participant.data.eventTimes(x).vasOnset = GetSecs; %WaitSecs('UntilTime', participant.data.eventTimes(x).trialStartTime+participant.settings.timings.stim(x)-participant.settings.ptb.slack);
        GetContinuousVASRatings(x,participant.data.eventTimes(x).vasOnset + 37); %show VAS for 37 sec
        
        %Screen('FillOval', participant.settings.window, participant.settings.conf.black, participant.settings.ptb.fix, participant.settings.ptb.fix_r+20);
        participant.data.eventTimes(x).vasEnd = GetSecs; %Screen('Flip',participant.settings.window);
    ...
    % get continuous VAS ratings -- call with tmax = 37 sec
    function GetContinuousVASRatings(tt,tmax)      
        tvoff     = -90;
       ...
       % SetMouse(mX, mY); % set mouse to cursorpos
        ...
        while tstamp < tmax
            ...
            participant.data.vasrate(tt, i) = (cursorpos(1) - participant.settings.vas.cposmaxl(1))/participant.settings.vas.cstep;
            participant.data.vas_timestamp(tt,i) = tstamp;
