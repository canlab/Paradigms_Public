function data = run_structural(subject,day)
...
%% main task loop
    ...
    for trial=1:participant.experiment.ntrials
        RunTrial(trial)
        save(fullfile(participant.settings.path.tmp,sprintf('sub%04d_day%d_structural_tmp.mat',subject,day)),'participant');
    end
    ...
    % finish up
    save(fullfile(participant.settings.path.data,sprintf('sub-%04d_ses-%02d_structural.mat',subject,day)),'participant');
    cleanup;
...
% These functions are used in the main task loop.
    ...
    % Psychtoolbox 3.0, and sets up communication with the MRI machine.
        ...
        participant.experiment.ntrials           = 14;
        ...
        participant.settings.ptb.qstr = {'How unpleasant?' 'How unpleasant?' 'How unpleasant?' 'How unpleasant?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?' 'How much back pain do you have right now?'};
        ...
        participant.settings.timings.stim = repmat(23,1,participant.experiment.ntrials);
    ...
    % Setup sprite objects for pain rating screen
        ...
        DrawFormattedText(participant.settings.ptb.vas_sprite(1),'more than',xr-30,yb+l1);
...
%                outp(participant.settings.conf.b_port,participant.settings.conf.b_sigstim); WaitSecs(0.05);outp(participant.settings.conf.b_port,0); % biopac trigger
            ...
            if x == 1
                fwrite(participant.settings.conf.udp_t, '0004, 0006, t');
                participant.data.eventTimes(x).vasOnset = WaitSecs('UntilTime', participant.data.trialStartTime(x)+ 7 - participant.settings.ptb.slack);
            elseif x == 2
                playblocking(experiment.players{1});
                participant.data.eventTimes(x).vasOnset = WaitSecs('UntilTime', participant.data.trialStartTime(x)+7 - participant.settings.ptb.slack);
            elseif x == 3 
                fwrite(participant.settings.conf.udp_t, '0007, 0006, t');
                participant.data.eventTimes(x).vasOnset = WaitSecs('UntilTime', participant.data.trialStartTime(x)+7 - participant.settings.ptb.slack);
            elseif x == 4
                playblocking(experiment.players{2});
                participant.data.eventTimes(x).vasOnset = WaitSecs('UntilTime', participant.data.trialStartTime(x)+7 - participant.settings.ptb.slack);
            else
                participant.data.eventTimes(x).vasOnset = WaitSecs('UntilTime', participant.data.trialStartTime(x)+participant.settings.timings.stim(x) - participant.settings.ptb.slack);
            end
            ...
            participant.data.eventTimes(x).trialEndTime = WaitSecs('UntilTime', participant.data.trialStartTime(x)+ 30 - participant.settings.ptb.slack);
        else
  ...
  %              outp(participant.settings.conf.b_port,participant.settings.conf.b_sigstim); WaitSecs(0.05);outp(participant.settings.conf.b_port,0); % biopac trigger
    ...
    % Get VAS Pain Ratings
        ...
        DrawFormattedText(participant.settings.window,participant.settings.ptb.qstr{tt},'center',participant.settings.ptb.mid(2)+tvoff);
