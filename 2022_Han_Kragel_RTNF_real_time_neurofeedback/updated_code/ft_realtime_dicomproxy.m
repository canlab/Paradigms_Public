...
while N==0
 ...
 pause(.2); %wait a bit so we dont have file access issues
...
while 1    
  ...
  if n==1
      ...
      mats=dir('*sn.mat');
      for i=1:length(mats)
         delete([mats(i).folder '\' mats(i).name]); 
      end
      niis=dir('*.nii');
       for i=1:length(mats)
         delete([niis(i).folder '\' niis(i).name]); 
      end
      
      estimate_norm(V)
      apply_deformation(folder);

      
      % D=spm_read_vols(V);
      % figure(1);imagesc(squeeze(D(:,:,30)));
  else
      % D=spm_read_vols(V);
      % figure(1);imagesc(squeeze(D(:,:,30)));
