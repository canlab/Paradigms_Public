...
key.ttl = KbName('5%');
key.s = KbName('s');
trial_times=[40 124 218 314 418 522 610];
mousebuttons=[1 3];
trial_start=9999;
jit=9999;

% start dicom streaming - will require user to select folder where DICOMs
% are getting written
...
while 1
    if (GetSecs-timing.ttl)>15
        break
    end; %wait 15 seconds to ensure that normalization, etc. has been done correctly
end
