...
% script that takes realtime buffer, preprocesses it, and computes NPS expression
%
...
% % get fear model
% beta_fear=spm_read_vols(spm_vol('C:\rtfMRI\dicom_streaming\wmean_3comp_fearful_group_emotion_PLS_beta_BSz_10000it.img'));
% beta_fear=beta_fear(:);
...
% fixation screen
Screen('TextSize',window,72);
DrawFormattedText(window,'Configuring feedback','center','center',255);
...
while 1
    ...
    S.TR=.460; %hard code this for now
    if isempty(S)
...
%             break;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % CALCULATE NPS EXPRESSION RELATIVE TO PERIOD AT BEGINNING OF SCANNING
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if numProper<16
            ...
            tOUT=OUT;
            tOUT=tOUT*1000; %deal with scaling issues
            if tOUT<=0
                tOUT=10;
            end
           % radius=(min(tOUT,700));
        ...
        if ci > 832 % change to manual close - this should be happening on compute PC indefinitely
            Screen('CloseAll');
            break;
        end
