...
try
    ...
    ljudObj.AddRequestS(ljhandle, 'LJ_ioPUT_AIN_RANGE', 0, LJ_rgBIP5V, 0, 0);
    ...
    for i=1:loopAmount
        ...
        % Displays the first scan.
        disp(['First scan = ' num2str(adblData(1)) ', ' num2str(adblData(2))])

        % Retrieve the current backlog. The UD driver retrieves stream data
        % from the UE9 in the background, but if the computer or code is too
        % slow the driver might not be able to read the data as fast as the UE9
        ...
        [ljerror, dblControlBacklog] = ljudObj.eGetSS(ljhandle, 'LJ_ioGET_CONFIG', 'LJ_chSTREAM_BACKLOG_CONTROL', dblControlBacklog, dummyDoubleArray);
        disp(['Control Backlog = ' num2str(dblControlBacklog)])
