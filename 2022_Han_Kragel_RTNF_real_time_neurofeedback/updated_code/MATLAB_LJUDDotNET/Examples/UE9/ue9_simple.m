...
try
    ...
    ljudObj.ePutSS(ljhandle, 'LJ_ioPUT_CONFIG', 'LJ_chAIN_RESOLUTION', 16, 0);

    % Configure the analog input range on channels 2 and 3 for
    % bipolar gain=1 (LJ_rgBIP5V = 3).
    LJ_rgBIP5V = ljudObj.StringToConstant('LJ_rgBIP5V');
    ljudObj.ePutS(ljhandle, 'LJ_ioPUT_AIN_RANGE', 2, LJ_rgBIP5V, 0);
    ljudObj.ePutS(ljhandle, 'LJ_ioPUT_AIN_RANGE', 3, LJ_rgBIP5V, 0);
    ...
    % Now we add requests to write and read I/O.  These requests will be
    % processed repeatedly by go/get statements in every interation of the
    % while loop below.
            ...
            try
                ...
                if(isa(e, 'NET.NetException'))
                    ...
                    if(isa(eNet, 'LabJack.LabJackUD.LabJackUDException'))
                        ...
                        if(double(eNet.LJUDError) == LJE_NO_MORE_DATA_AVAILABLE)
                            finished = true;
                        end
                    end
                end
