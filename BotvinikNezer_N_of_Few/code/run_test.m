function part2_test_data_table = run_test(p,subject_num,session_num,use_biopac,debugging_mode,participant_parameters, num_blocks, sites_order)
% code by Rotem Botvinik-Nezer
% based on codes by Heejung Jung and Xiaochun Han
% Last updated January 2021
%
% This function runs the test task of the cue-pain part of the NOF experiment
% with heat stimuli and pain and expectation ratings.
% All cues are presented in a random order. Participants rate expectations,
% then the same heat stimuli are delivered, and participants rate pain.
% One block (once each cue, with 47C) for sessions 1-6.
% Three blocks for sessions 7-10.
% Instructions for the "instructed cues" ("L"/"H") are only given at the
% beginning of sessions 1-6.
%
...
% Cue, expectation and confidence ratings, "Get ready!", pre-stim fixation,
% heat stim, post-stim fixation, pain rating, ISI fixation.
%
% Input:
...
%
% Output:
% part2_test_data_table: the table with all the information about the task,
...
all_cues_with_dummy = {'dog','cat','truck','car','low','high','neutral','dummy_no_cue'};
cues_types = cell(size(all_cues_with_dummy));
cues_types(end-3:end) = {'instruct_low','instruct_high','neutral','dummy_no_cue'};
for cue_ind = 1:4
...
num_rep_per_block = 1; % how many times each cue is presented in each block
exp_trials_per_block = length(all_cues) * num_rep_per_block; % not including the dummy trial
...
if session_num <= 6
    heat_temps = 47 + participant_parameters.calibration_factor;
else
    heat_temps = [46.5,47,47.5] + participant_parameters.calibration_factor; % temps will be randomized across all blocks
end
...
all_cues_per_block = repelem(1:length(all_cues), num_rep_per_block);
for block_ind = 1:num_blocks
    cues_order(:,block_ind) = [dummy_cue_id, all_cues_per_block(randperm(length(all_cues_per_block)))];
end
% now, randomize temps across blocks for each temp. Don't allow the same
% temp to repeat in more than 2 consecutive trials
temps_order = zeros(size(cues_order));
temps_order(cues_order == dummy_cue_id) = dummy_temp;
if length(heat_temps) > 1
    max_strike_valid = 2;
    valid_order = 0;
    while ~ valid_order
        for cue_ind = 1:length(all_cues)
            temps_order(cues_order ==cue_ind) = heat_temps(randperm(length(heat_temps)));
        end
        valid_order = 1;
        for trial_ind = 1:size(temps_order,1)-max_strike_valid
            if unique(temps_order(trial_ind:trial_ind+max_strike_valid, block_ind)) == 1
                valid_order = 0;
            end
        end
    end
...
biopac_signal_out.cue_instruct_low_start = 18;
biopac_signal_out.cue_instruct_high_start = 19;
biopac_signal_out.cue_neutral_start = 20;
...
cues_pics = {imread([cues_dir filesep all_cues{1} '.png'])
             imread([cues_dir filesep all_cues{2} '.png'])
             imread([cues_dir filesep all_cues{3} '.png'])
             imread([cues_dir filesep all_cues{4} '.png'])
             imread([cues_dir filesep all_cues{5} '.png'])
             imread([cues_dir filesep all_cues{6} '.png'])
             imread([cues_dir filesep all_cues{7} '.png'])};
             
% cues_pics.dog = imread([cues_dir filesep 'dog.png']);
% cues_pics.cat = imread([cues_dir filesep 'cat.png']);
% cues_pics.truck = imread([cues_dir filesep 'truck.png']);
% cues_pics.car = imread([cues_dir filesep 'car.png']);
% cues_pics.low = imread([cues_dir filesep 'low.png']);
% cues_pics.high = imread([cues_dir filesep 'high.png']);
% cues_pics.neutral = imread([cues_dir filesep 'neutral.png']);
...
var_names = {'subject_id','session','task_onset','block',...
    ...
    'heat_onset','heat_temp','heat_peak_duration','heat_ramp_up_rate',...
    'heat_ramp_down_rate','total_stim_duration','program_num','medocResponseStr_choose_program','medocResponseStr_trigger',...
...
var_types = {'string','double','double','double',...
    ...
    'cell','cell','cell'};
part2_test_data_table = table('Size',[total_trials_per_block*num_blocks,length(var_names)],'VariableTypes',var_types,'VariableNames',var_names);
part2_test_data_table.subject_id(:) = subject_id;
part2_test_data_table.session(:) = session_num;
part2_test_data_table.block(:) = repelem(1:num_blocks,total_trials_per_block);
part2_test_data_table.site(:) = repelem(sites_order,total_trials_per_block);
part2_test_data_table.trial(:) = repmat(1:total_trials_per_block,1,num_blocks);
part2_test_data_table.get_ready_duration(:) = get_ready_duration;
part2_test_data_table.pre_stim_fixation_duration(:) = pre_stim_durations(:);
part2_test_data_table.post_stim_fixation_duration(:) = post_stim_fixation_durations(:);
part2_test_data_table.heat_temp(:) = temps_order(:);
part2_test_data_table.heat_peak_duration(:) = heat_peak_duration;
part2_test_data_table.heat_ramp_up_rate(:) = ramp_up_rate;
part2_test_data_table.heat_ramp_down_rate(:) = ramp_down_rate;
part2_test_data_table.cue_duration(:) = cue_duration;
part2_test_data_table.cue(:) = all_cues_with_dummy(cues_order(:));
part2_test_data_table.cue_type(:) = cues_types(cues_order(:));
part2_test_data_table.use_biopac(:) = use_biopac;
writetable(part2_test_data_table, data_filename);

%% --------------------------- Load instructions -------------------------- 
instruct_filepath = [main_dir filesep 'instructions'];
if session_num <= 6
    instruct_task_start = fullfile(instruct_filepath, 'cue_pain_test_start_with_instruct.png');
else
    instruct_task_start = fullfile(instruct_filepath, 'cue_pain_test_start_without_instruct.png');    
end
...
for block_num = 1:num_blocks
    ...
    if block_num == 1
        task_onset = GetSecs;
        part2_test_data_table.task_onset(:) = 0; % task_onset is used as the anchor for all other timings
        if use_biopac
    ...
    part2_test_data_table.start_sound_onset(1+total_trials_per_block*(block_num-1)) = start_sound_onset-task_onset;
    WaitSecs(2);
    ...
    block_onset = GetSecs;
    part2_test_data_table.block_onset(1+(total_trials_per_block*(block_num-1)):total_trials_per_block*(block_num)) = block_onset - task_onset;
    if use_biopac
    ...
    for trial_num = 1:total_trials_per_block
        ...
        if trial_num == ceil(total_trials_per_block / 2)
            part2_test_data_table.middle_sound_onset(trial_table_ind) = GetSecs-task_onset;
            sound(gopro_hilight, Fs_gopro_hilight);
            if use_biopac
        ...
        trial_onset = GetSecs;
        part2_test_data_table.trial_onset(trial_table_ind) = trial_onset - task_onset;
        if use_biopac
            biopac_signal(biopac_signal_out.trial_start);
        end
        
        %% choose a program for the thermode and start it (no trigger yet)
        [thermode_program, total_stim_duration, responseStr_choose_program] = thermode_choose_program(experiment_name, part2_test_data_table.heat_temp(trial_table_ind), part2_test_data_table.heat_peak_duration(trial_table_ind), part2_test_data_table.heat_ramp_up_rate(trial_table_ind), part2_test_data_table.heat_ramp_down_rate(trial_table_ind), main_dir);
        part2_test_data_table.program_num(trial_table_ind) = thermode_program;
        part2_test_data_table.total_stim_duration(trial_table_ind) = total_stim_duration;
        part2_test_data_table.medocResponseStr_choose_program(trial_table_ind) = responseStr_choose_program;
        
        %% cue
        if trial_num > num_dummy_trials_per_block % show cue and expectation rating only if it's not a dummy trial
            start.texture = Screen('MakeTexture',p.ptb.window, cues_pics{cues_order(trial_num,block_num)});
            Screen('DrawTexture',p.ptb.window,start.texture,[],[]);
            cue_onset = Screen('Flip',p.ptb.window);
            if use_biopac
                switch part2_test_data_table.cue_type{trial_table_ind}
                    case 'conditioning_low'
                        biopac_signal(biopac_signal_out.cue_conditioning_low_start);
                    case 'conditioning_high'
                    ...
                    case 'instruct_low'
                        biopac_signal(biopac_signal_out.cue_instruct_low_start);
                    case 'instruct_high'
                        biopac_signal(biopac_signal_out.cue_instruct_high_start);
                    case 'neutral'
                        biopac_signal(biopac_signal_out.cue_neutral_start);
                end
            end
            part2_test_data_table.cue_onset(trial_table_ind) = cue_onset - task_onset;
            ...
            part2_test_data_table.pain_expect(trial_table_ind) = pain_expect;
            part2_test_data_table.pain_expect_onset(trial_table_ind) = rating_onset-task_onset;
            %task1_data_table.pain_expect_duration(trial_table_ind) = RT;
            part2_test_data_table.pain_expect_trajectory{trial_table_ind} = trajectory;
            part2_test_data_table.pain_expect_response_onset(trial_table_ind) = response_onset-task_onset;
            ...
            part2_test_data_table.expect_confidence(trial_table_ind) = expect_confidence;
            part2_test_data_table.expect_confidence_onset(trial_table_ind) = rating_onset-task_onset;
            %task1_data_table.expect_confidence_duration(trial_table_ind) = RT;
            part2_test_data_table.expect_confidence_trajectory{trial_table_ind} = trajectory;
            part2_test_data_table.expect_confidence_response_onset(trial_table_ind) = response_onset-task_onset;
        end
        ...
        part2_test_data_table.get_ready_onset(trial_table_ind) = get_ready_onset - task_onset;
        ...
        part2_test_data_table.pre_stim_fixation_onset(trial_table_ind) = pre_stim_fixation_onset - task_onset;
        ...
        if use_biopac
            biopac_signal(biopac_signal_out.stimulus_temps_signals(biopac_signal_out.potential_temps == part2_test_data_table.heat_temp(trial_table_ind)));
        end
        [heat_onset, responseStr_trigger] = thermode_trigger(thermode_program, total_stim_duration); % delivers the heat stimuli
        part2_test_data_table.heat_onset(trial_table_ind) = heat_onset - task_onset;
        part2_test_data_table.medocResponseStr_trigger(trial_table_ind) = responseStr_trigger;
        ...
        part2_test_data_table.post_stim_fixation_onset(trial_table_ind) = post_stim_fixation_onset - task_onset;
        ...
        part2_test_data_table.pain_rating(trial_table_ind) = pain_rating;
        part2_test_data_table.pain_rating_onset(trial_table_ind) = rating_onset-task_onset;
        %task1_data_table.pain_rating_duration(trial_table_ind) = RT;
        part2_test_data_table.pain_rating_trajectory{trial_table_ind} = trajectory;
        part2_test_data_table.pain_rating_response_onset(trial_table_ind) = response_onset-task_onset;
        ...
        part2_test_data_table.ISI_fixation_onset(trial_table_ind) = ISI_fixation_onset - task_onset;
        ...
        part2_test_data_table.trial_offset(trial_table_ind) = trial_offset - task_onset;
    ...
    part2_test_data_table.end_sound_onset(trial_table_ind) = end_sound_onset-task_onset;
    WaitSecs(2);
...
%% save final table as a .csv and as a .mat file
if use_biopac
    biopac_signal(biopac_signal_out.task_end);
end
writetable(part2_test_data_table, data_filename);
save([data_filename(1:end-4) '_' timestamp], 'part2_test_data_table');
