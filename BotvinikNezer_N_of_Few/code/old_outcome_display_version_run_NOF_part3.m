...
pair_duration = 2; % duration of binary options display, in secs
lottery_duration = 1.5;
outcome_duration = 2; % duration of outcome presentation, in secs
...
for block_num = 1:num_blocks_per_session
    ...
    for trial_num = 1:total_trials_per_block
        ...
        if ~dummy_trial %  display pair and ask for expectation ratings only if it's not a dummy trial
            ...
            random_num = randi(2);
            if random_num == 1
                while GetSecs < lottery_onset + lottery_duration
                    Screen('PutImage', p.ptb.window, left_stim_pic);
                    Screen('Flip',p.ptb.window);
                    WaitSecs(0.1);
                    Screen('PutImage', p.ptb.window, right_stim_pic);
                    Screen('Flip',p.ptb.window);
                    WaitSecs(0.1);
                end
            else
            ...
            %% display outcome
            Screen('PutImage', p.ptb.window, outcome_pic);
